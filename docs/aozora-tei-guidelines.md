...
3. カードの「作品データ」「底本データ」を確認し、TEI の `sourceDesc` に転記される
   ...
   から自動転記する)
4. 著作権: 著者・翻訳者・校訂者すべての保護期間満了を確認(青空文庫収録済みでも
   **保護期間中の作品はカードだけ存在して本文非公開**のことがある。TPP整合法の延長で
   1968年以降没の作家(志賀直哉・川端康成など)は当面使えない)
5. **旧フォーマット(2001年前後公開)の XHTML** — `main_text` も
   `bibliographical_information` も無く、本文は `</H2>`〜`<HR>`、ルビは
   `<!R>親（よみ）`、外字は本文中 ※ + 末尾に画像。パイプラインは自動で
   フォールバックする(`extract_rich_legacy`)。**外字は自動解決できないので
   `fixes` で手当てする**(例: 堺事件の ※→揷)

## 2. 本文抽出の方針(パイプラインが実施 — 変えない)

- **ルビは TEI P5 の `<ruby><rb>親文字</rb><rt>読み</rt></ruby>` として保持する**。
  固有表現とは入れ子にする(`persName > ruby` / 親文字が固有表現より広ければ
  `ruby > rb > persName`)。固有表現の境界をまたぐルビ(交差)だけは入れ子に
  できないので落とす(ビルド時に件数を表示)
  - 固有表現の辞書マッチは**親文字だけを連結した本文**に対して行うので、
    ルビの有無はマッチ結果に影響しない
  - 外字の ※ がルビ親文字の末尾に入り、注記 `［＃…］` が `</ruby>` の外に
    置かれる書き方があるため、外字解決はルビ区間をまたいで行う(実装済み)
  - 取り込み側(archivebase)は `<rt>` を落としてから全文検索・固有表現集計に
    かけるので、読みが本文に混入しない
...
- **このタグ語彙は表示側との契約でもある**: archivebase の単一スタイルシート
  ...
  巻末台帳・ルビを描画する。**語彙を増やす場合は tei-aozora.xsl も同時に更新する**
...
2. 回避策は 3 つ:
   - (a) **より長い表層形にする**(太田豊太郎 → 太田 の順で重ねる、湯殿山 → 湯殿)
   - (b) **`skip` で誤マッチの文脈だけを抑止する** — 誤マッチする箇所を含む長い文字列を
     `["skip", "仁をもって衛となした", null]` として tags に足すと、その領域だけ
     タグ付けが止まる(タグは出ない)。**表層形ごと捨てる前に必ずこれを検討する**。
     例: 『弟子』の「衛」は29件中1件だけ「仁をもって衛となした」(=守り)に誤マッチ
     するが、skip 1件で残り28件の地名を回収できた
   - (c) それでも無理なら除外し、レポートに理由と失う件数を書く
3. 実際に踏んだ罠の例(除外判断の参考):
...
```jsonc
{
  ...
  "tags": [["pers", "良平", "ryohei"],
           ...
           ["skip", "仁をもって衛となした", null]]  // この領域はタグ付けしない(§5-2b)
}
```
